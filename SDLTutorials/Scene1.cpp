...
bool Scene1::OnCreate() {
	...
	backgroundCanvasEntity = scene1System.CreateEntity();
	labelEntity = scene1System.CreateEntity();
	timeLabelEntity = scene1System.CreateEntity();
	...
	for (int i = 0; i < 8; i++) {
		itemButtons.emplace_back(std::make_shared<UIButtonEntity>((150 * i) + 50, 600, 125, 50, items[i], GameManager::assets.GetFont("Bahnschrift_14")));
	}
	...
	for (int i = 0; i < 8; i++) {
		scene1System.AddEntity(itemButtons[i]);
	}
	
	scene1System.AddEntity(barEntity);
	...
	for (int i = 0; i < 8; i++) {
		itemButtons[i]->SetColor(itemButtons[i]->e_Normal, black, slateBlue);
		itemButtons[i]->SetColor(itemButtons[i]->e_Hover, darkGrey, lightGreen);
		itemButtons[i]->SetColor(itemButtons[i]->e_MouseDown, white, evergreen);
		itemButtons[i]->SetColor(itemButtons[i]->e_Active, white, black);
	}
